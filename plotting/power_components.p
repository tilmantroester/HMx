...
if(print==0){set term aqua dashed dl 1 font ',14'}
if(print==1){set term post enh col font ',14' size 7,7; set output 'paper/power_components.eps'}
...
# Key position
if(print==0) {set key top left}
if(print==1) {set key at screen 0.52,0.96}
...
plot NaN w l lw 3 lc -1 dt 2 ti 'Two-halo term',\
     ...
     power(z,1,6) u 1:(f1*$5) w l lc 6 dt 1 lw 3 ti 'Pressure [meV cm^{-3}]',\
