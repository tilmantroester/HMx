...
power_zsim(dir,mod,n,m,c,f1,f2,iz)=sprintf('%s/%s_n%d_m%d_c%d_best_cos1_%d%d_z%d.dat',dir,mod,n,m,c,f1,f2,iz)
...
if(iplot==2) {print 'Example file: ', power_zsim(dir,word(mods,2),n,m,c,f1,f2,1)}
print ''
...
set obj rect from kmin_fit,-1 to kmin,35
...
if(iplot==2){
plot for [i=1:words(mods)] NaN w l lw 2 lc rgb word(cols,i) ti word(names,i),\
     for [i=1:words(mods)] power_zsim(dir,word(mods,i),n,m,c,f1,f2,iz) u 1:($2/$1**1.5) w p pt 7 ps .5 lc rgb word(cols,i) noti,\
     for [i=1:words(mods)] power_zsim(dir,word(mods,i),n,m,c,f1,f2,iz) u 1:($3/$1**1.5) w l dt 1 lw 2  lc rgb word(cols,i) noti
}
