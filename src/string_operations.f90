...
!!$    IF(i<0) STOP 'NUMBER_FILE: Error: cannot write negative number file names'
!!$
!!$    IF(num>maxnum) STOP 'NUMBER_FILE: Error: need to add extra number capacity'
!!$    
!!$    IF(num==1) THEN
!!$
!!$       IF(i>=10) STOP 'Error: i is too large a number for space (i>num)'
!!$       WRITE(num1,fmt='(I0.1)') i
!!$       number_file_zeroes=TRIM(fbase)//num1//TRIM(fext)
!!$
!!$    ELSE IF(num==2) THEN
!!$
!!$       IF(i>=100) STOP 'Error: i is too large a number for space (i>num)'
!!$       WRITE(num2,fmt='(I0.2)') i
!!$       number_file_zeroes=TRIM(fbase)//num2//TRIM(fext)
!!$
!!$    ELSE IF(num==3) THEN
!!$
!!$       IF(i>=1000) STOP 'Error: i is too large a number for space (i>num)'
!!$       WRITE(num3,fmt='(I0.3)') i
!!$       number_file_zeroes=TRIM(fbase)//num3//TRIM(fext)
!!$       
!!$    ELSE IF(num==4) THEN
!!$
!!$       IF(i>=10000) STOP 'Error: i is too large a number for space (i>num)'
!!$       WRITE(num4,fmt='(I0.4)') i
!!$       number_file_zeroes=TRIM(fbase)//TRIM(num4)//TRIM(fext)
!!$
!!$    END IF
